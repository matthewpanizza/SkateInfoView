...
// ESP32 IDF SPI Helper Macros
#define MCP_LOG_TAG     "MCP2515"
#define SPI_SPEED_HZ (10000000)  // 10 MHz SPI clock
...
void MCP_CAN::mcp2515_reset(void)                                      
{
    ...
    spi_device_transmit(mcpSPI, &trans);
    
    vTaskDelay(pdMS_TO_TICKS(5)); // If the MCP2515 was in sleep mode when the reset command was issued then we need to wait a while for it to reset properly
}
...
{
    INT8U tx_buf[3] = {MCP_READ, address, 0x00};
    INT8U rx_buf[3];
    ...
    return rx_buf[2];  // Return the third byte (data from register)
}
...
void MCP_CAN::mcp2515_readRegisterS(const INT8U address, INT8U values[], const INT8U n)
{
    ...
    // Build transmit buffer: MCP_READ, address, followed by dummy bytes
    tx_buf[0] = MCP_READ;
    tx_buf[1] = address;
    for (int i = 0; i < n; i++) {
        tx_buf[2 + i] = 0x00;  // Dummy bytes for reading
    }
    
    spi_transaction_t trans = {
        .length = (size_t)((2 + n) * 8),  // Total bits to transmit
        .tx_buffer = tx_buf,
        .rx_buffer = rx_buf
    };
    ...
    // Copy received data (skip MCP_READ and address bytes)
    for (int i = 0; i < n; i++) {
        values[i] = rx_buf[2 + i];
    }
}
...
void MCP_CAN::mcp2515_setRegisterS(const INT8U address, const INT8U values[], const INT8U n)
{
    ...
    // Build transmit buffer: MCP_WRITE, address, followed by values
    tx_buf[0] = MCP_WRITE;
    tx_buf[1] = address;
    for (int i = 0; i < n; i++) {
        tx_buf[2 + i] = values[i];
...
{
    INT8U tx_buf[2] = {MCP_READ_STATUS, 0x00};
    INT8U rx_buf[2];
    ...
    return rx_buf[1];  // Return the second byte (status register data)
}
...
INT8U MCP_CAN::mcp2515_requestNewMode(const INT8U newmode)
{
    ...
    while(1)
    {
        ...
        if((statReg & MODE_MASK) == newmode) // We're now in the new mode
            return MCP2515_OK;
        else if((esp_timer_get_time() - startTime) > 200000) // Wait no more than 200ms (200000 us)
            return MCP2515_FAIL;
    }
}
...
INT8U MCP_CAN::mcp2515_init(const INT8U canIDMode, const INT8U canSpeed, const INT8U canClock)
{
  ...
  INT8U res;
    gpio_set_level(GPIO_NUM_10, 1);  // CS high initially (inactive)
    mcp2515_reset();
    ...
    {
#if DEBUG_MODE
    ESP_LOGE(MCP_LOG_TAG, "Entering Configuration Mode Failure...");
#endif
      return res;
    }
#if DEBUG_MODE
    ESP_LOGI(MCP_LOG_TAG, "Entering Configuration Mode Successful!");
#endif
    ...
    {
#if DEBUG_MODE
    ESP_LOGE(MCP_LOG_TAG, "Setting Baudrate Failure...");
#endif
      return res;
    }
#if DEBUG_MODE
    ESP_LOGI(MCP_LOG_TAG, "Setting Baudrate Successful!");
#endif
...
*/
            ...
            default:
#if DEBUG_MODE        
    ESP_LOGE(MCP_LOG_TAG, "Setting ID Mode Failure...");
#endif           
            return MCP2515_FAIL;
            break;
}    
        ...
        {
#if DEBUG_MODE       
    ESP_LOGE(MCP_LOG_TAG, "Returning to Previous Mode Failure...");
#endif           
          return res;
        }
...
** Descriptions:            Public function to declare CAN class with SPI handle. CS handled by SPI HAL
*********************************************************************************************************/
MCP_CAN::MCP_CAN(spi_device_handle_t _SPI)
{
...
INT8U MCP_CAN::init_Mask(INT8U num, INT8U ext, INT32U ulData)
{
...
#if DEBUG_MODE
    ESP_LOGI(MCP_LOG_TAG, "Starting to Set Mask!");
#endif
    res = mcp2515_setCANCTRL_Mode(MODE_CONFIG);
    if(res > 0){
#if DEBUG_MODE
    ESP_LOGI(MCP_LOG_TAG, "Entering Configuration Mode Failure...");
#endif
	return res;
     }
    ...
    if(res > 0){
#if DEBUG_MODE
    ESP_LOGE(MCP_LOG_TAG, "Entering Previous Mode Failure...");
    ESP_LOGE(MCP_LOG_TAG, "Setting Mask Failure...");
#endif
	return res;
    }
#if DEBUG_MODE
    ESP_LOGI(MCP_LOG_TAG, "Setting Mask Successful!");
#endif
    return res;
}
...
INT8U MCP_CAN::init_Filt(INT8U num, INT8U ext, INT32U ulData)
{
    INT8U res = MCP2515_OK;
#if DEBUG_MODE
    ESP_LOGI(MCP_LOG_TAG, "Starting to Set Filter!");
#endif
    res = mcp2515_setCANCTRL_Mode(MODE_CONFIG);
    if(res > 0)
    {
#if DEBUG_MODE
      ESP_LOGE(MCP_LOG_TAG, "Enter Configuration Mode Failure..."); 
#endif
      return res;
    }
...
#if DEBUG_MODE
    ESP_LOGE(MCP_LOG_TAG, "Entering Previous Mode Failure..."); 
	ESP_LOGE(MCP_LOG_TAG, "Setting Filter Failure...");
#endif
      return res;
    }
#if DEBUG_MODE
    ESP_LOGI(MCP_LOG_TAG, "Setting Filter Successful!");
#endif
...
INT8U MCP_CAN::sendMsg()
{
    ...
    temp = esp_timer_get_time(); // microseconds
